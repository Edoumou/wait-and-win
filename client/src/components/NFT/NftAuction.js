...
class NftAuction extends Component {
    ...
    componentDidMount = async () => {
        ...
        let auctionState = await this.props.NFTContract.methods.auctionState().call();
        let highestBidder = await this.props.NFTContract.methods.highestBidder().call();

        let highestBindingBid = await this.props.NFTContract.methods.highestBindingBid().call();
    ...
    onButtonClick = async e => {
        ...
        await this.props.NFTContract.methods
            .mintImage(CID).send({ from: this.props.account });
        ...
        let imageID = await this.props.NFTContract.methods
            .imageIDs(CID).call();
        let imageCID = await this.props.NFTContract.methods.images(imageID).call();

        // get the owner of the image from the contract
        const owner = await this.props.NFTContract.methods.ownerOf(imageID).call();
    ...
    onBidButtonClick = async () => {
        ...
        await this.props.NFTContract.methods.placeBid()
    ...
    onAuctionEnded = async () => {
        ...
        await this.props.NFTContract.methods.endAuction()
    ...
    onAuctionTerminated = async () => {
        ...
        await this.props.NFTContract.methods.finalizeAuction(this.state.id)
            .send({ from: this.props.account });
        ...
        const owner = await this.props.NFTContract.methods.ownerOf(this.state.id).call();
        this.setState({
            owner,
        ...
        return (
            <div className="ico">
                <div className="token-info">
                    ...
                    <h3>highest bidder: {FormateAddress(this.state.highestBidder)} </h3>
                    ...
                    <span className='input-btn'>
                        <Input size='mini' placeholder='Image ID' onChange={e => { this.setState({ id: e.target.value }) }}>
                        </Input>
                    </span>
                    {
                        this.state.id === undefined || this.state.id === null ?
                            <Button disabled size='large' color='blue' onClick={this.onAuctionTerminated}>
                                claim the NFT or refund
                            </Button>
                            :
                            <Button size='large' color='blue' onClick={this.onAuctionTerminated}>
                                claim the NFT or refund
                            </Button>
                    }
                </div>
                    ...
                    <Grid columns={3} celled stackable>
                        <Grid.Row>
                            ...
                            <Grid.Column textAlign='center' width={4}>
                                ...
                                <br></br>
                                <div className='token-buy-input'>
                                    <Input fluid labelPosition='right' type='text' placeholder='min: 0.1 ETH'>
                            ...
                            <Grid.Column textAlign='center' width={8}>
                                ...
                                <br></br>
                                owner: <strong>{FormateAddress(this.state.owner)}</strong>
                                <br></br>
                                {
                                    this.state.imageID !== '' ?
